...
class TrialEnv(BaseEnv):
    ...
    def view_ob(self, period=None):
        ...
        if period is None:
            ...
            return self.ob[self.start_ind[period]:self.end_ind[period]] ## 该period的start到end的ob
    ...
    def _add_ob(self, value, period=None, where=None, reset=False):
        ...
        # self.ob[self.start_ind[period]:self.end_ind[period]] = values
        ...
        if where is None:
            ...
            if reset:
                ob[..., where] *= 0
            try:
                ob[..., where] += value(ob[..., where]) # ...: 省略前面的维度
            except TypeError:
                ob[..., where] += value
    ...
    def add_randn(self, mu=0, sigma=1, period=None, where=None):
        ...
        if where is None:
            ...
            if isinstance(where, str):
                where = self.observation_space.name[where]
            # TODO: This only works if the slicing is one one-dimension
            ob[..., where] += mu + self.rng.randn(*ob[..., where].shape) * sigma # noise: mu + sigma, 每个时间点的sigma不同

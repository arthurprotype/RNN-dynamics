...
class MyTask(ngym.TrialEnv):
    def __init__(self, dt=100, rewards=None, timing=None, sigma=1, dim_ring=2, theta=0):
        ...
        self.theta = np.linspace(0, 2 * np.pi, dim_ring+1)[:-1] # dim_ring+1==3, 在线性空间中以均匀步长生成数字序列
        ...
        name = {'fixation': 0, 'stimulus1': range(1, 3), 'stimulus2': range(3, 5), 'stimulus3': range(5, 7), 'stimulus4': range(7, 9), 'cue': range(9, 13)}
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(11+dim_ring,), dtype=np.float32, name=name)
    ...
    def _new_trial(self, **kwargs):
        ...
        gt3 = self.rng.choice(self.choices)
        gt4 = self.rng.choice(self.choices)
        trial = {
            'cue1': cue1,
            'cue2': cue2,
            'cue3': cue3,
        ...
        stim3_theta = random.choice(self.theta) 
        stim4_theta = random.choice(self.theta) 
        ...
        #     # if ground_truth == 0:
        #     #     difference_theta = - self.rng.choice(difference_list)
        #     # else:
        #     #     difference_theta = self.rng.choice(difference_list)
        #     # test_theta = stim1_theta + difference_theta
        ...
        #     # else:
        #     #     difference_theta = self.rng.choice(difference_list)
        #     # test_theta = stim2_theta + difference_theta
      
        # # trial['difference'] = difference_theta
        ...
        trial['stim3_theta'] = stim3_theta
        trial['stim4_theta'] = stim4_theta
        ...
        stim3 = np.cos(self.theta - stim3_theta)   
        stim4 = np.cos(self.theta - stim4_theta)  
        test = np.cos(self.theta - test_theta)  

        periods = ['sample1', 'delay1', 'sample2', 'delay2', 'sample3', 'delay3', 'sample4', 'delay4', 'cue', 'probe', 'decision']
        self.add_period(periods)

        self.add_ob(1, period=['sample1', 'delay1', 'sample2', 'delay2', 'sample3', 'delay3', 'sample4', 'delay4', 'cue', 'probe'], where='fixation') 
        self.add_ob(stim1, 'sample1', where='stimulus1')
        self.add_ob(stim2, 'sample2', where='stimulus2')
        self.add_ob(stim3, 'sample3', where='stimulus3')
        self.add_ob(stim4, 'sample4', where='stimulus4')
        self.add_ob([cue1,cue2,cue3,cue4], 'cue', where='cue')
        ...
        if cue1 == 1:
            ...
            self.add_ob(test, 'probe', where='stimulus3')
        else:
            self.add_ob(test, 'probe', where='stimulus4')
        self.add_randn(0, self.sigma, 'sample1', where='stimulus1')
        self.add_randn(0, self.sigma, 'sample2', where='stimulus2')
        self.add_randn(0, self.sigma, 'sample3', where='stimulus3')
        self.add_randn(0, self.sigma, 'sample4', where='stimulus4')
        self.add_randn(0, self.sigma, 'probe', where='stimulus1') 
        self.add_randn(0, self.sigma, 'probe', where='stimulus2') 
        self.add_randn(0, self.sigma, 'probe', where='stimulus3') 
        self.add_randn(0, self.sigma, 'probe', where='stimulus4') 

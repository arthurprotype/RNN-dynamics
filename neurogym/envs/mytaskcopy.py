...
class MyTaskCopy(ngym.TrialEnv):
    def __init__(self, dt=100, rewards=None, timing=None, sigma=1, dim_ring=2, cohs=None, theta=0):
        ...
        self.sigma = 0.1 * sigma / np.sqrt(self.dt)  # Input noise (调小一些)
        ...
        name = {'fixation': 0, 
                ...
                'test': range(1+dim_ring*2, 1+dim_ring*3),
                'cue': range(1+dim_ring*3, 3+dim_ring*3),
                }
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(3+dim_ring*3,), dtype=np.float32, name=name)
    ...
    def _new_trial(self, **kwargs):
        ...
        stim1 = np.cos(self.theta - stim1_theta) * 0.5 + 0.5
        stim2 = np.cos(self.theta - stim2_theta) * 0.5 + 0.5
        test = np.cos(self.theta - test_theta) * 0.5 + 0.5

        periods = ['fixation', 'sample1', 'delay1', 'sample2', 'delay2', 'cue', 'probe', 'decision']
        self.add_period(periods)

        self.add_ob(1, period=['fixation', 'sample1', 'delay1', 'sample2', 'delay2', 'cue', 'probe'], where='fixation') 
        ...
        self.add_ob(test, 'probe', where='test')
        ...
        self.add_randn(0, self.sigma, 'probe', where='test')
        ...
        if cue1 == 1:
            ...
            self.add_randn(0, self.sigma, 'probe', where='stimulus2') # 加噪音

...
class PerceptualDecisionMaking(ngym.TrialEnv): # 父类: TrialEnv
    ...
    def __init__(self, dt=100, rewards=None, timing=None, cohs=None,
        ...
        if rewards:
            self.rewards.update(rewards) # update: 把字典 y 中的项目插入字典 x

        self.timing = {
            'fixation': 100, # 100 2000 100
            'stimulus': 2000,
            'delay': 500,
            'decision': 1000}
        if timing:
        ...
        self.choices = np.arange(dim_ring) # [0,1]
        # self.choices = [1, 2]

        name = {'fixation': 0, 
                'stimulus': range(1, dim_ring+1)} # observation 观察到fixation & stim
        self.observation_space = spaces.Box(
            -np.inf, np.inf, shape=(1+dim_ring,), dtype=np.float32, name=name) ##np.inf:正无穷，浮点型
    ...
    def _new_trial(self, **kwargs):
        ...
        } # 随机选取ground_truth及任务难度
        trial.update(kwargs)

        coh = trial['coh']
        ground_truth = trial['ground_truth']
        stim_theta = self.theta[ground_truth] ##序列中gt对应的值 0 or pi, stimulus和gt有关，与gt差值越大，stim越大，stim应该是随机的?
        ...
        # Observations
        self.add_ob(1, period=['fixation', 'stimulus', 'delay'], where='fixation') ## decision之前不respond
    ...
    def _step(self, action):
        ...
        if self.in_period('fixation'):
            if action != 0:  # action = 0 means fixating
                ...
                reward += self.rewards['abort'] # 不该有action的时候有action就abort
            ...
            if action != 0:
                ...
                if action == gt:
                    ...
                    reward += self.rewards['fail'] # action == 0 reward == ?

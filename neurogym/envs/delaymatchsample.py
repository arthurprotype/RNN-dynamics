...
class DelayMatchSample(ngym.TrialEnv):
    ...
    def __init__(self, dt=100, rewards=None, timing=None, sigma=1.0,
        ...
        # self.theta = [0.3 * np.pi, 0.7 * np.pi]
    ...
    def _new_trial(self, **kwargs):
        ...
        trial = {
            ...
            'stim_theta': self.rng.choice(self.theta),
        }
        trial.update(kwargs)

        ground_truth = trial['ground_truth']
        stim_theta = trial['stim_theta']
        if ground_truth == 1:
            test_theta = stim_theta
        else:
            # test_theta = np.mod(stim_theta + np.pi, 2 * np.pi)
            test_theta = np.pi - stim_theta

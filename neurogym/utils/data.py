...
class Dataset(object):
    ...
    def __init__(self, env, env_kwargs=None,
        ...
        if isinstance(env, gym.Env):
            self.envs = [copy.deepcopy(env) for _ in range(batch_size)] ## 创造batch_size个env
        else:
        ...
        env = self.envs[0]
        self.env = env ##batch_size个env中的第一个env
        ...
        if cache_len is None:
            ...
            cache_len /= (np.prod(obs_shape) + np.prod(action_shape)) ##100,000/1600
            cache_len /= batch_size ## 62.5/16 ?
